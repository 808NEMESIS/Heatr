...
async def enrich_company(
    ...
    from utils.text_normalizer import (
        normalize_generated_text, validate_opener_sendable, strip_unverified_claims,
    )
    ...
    # QA-gate (fail-closed): ongetoetste website-claims eruit strippen, dan checken
    # dat de opener verzendbaar is (niet afgekapt, geen kliniek-stem, niet te kort)
    # — anders NIET opslaan, zodat kapotte/onbewezen output nooit stil een
    # 'productie-opener' wordt (root cause 2026-07-15).
    if opener_ok:
        opener_clean = strip_unverified_claims(opener_clean)
...
async def generate_personalized_opener(
    ...
    system_prompt = (
        ...
        "- Precies ÉÉN zin, 12-28 woorden, eindigend op een punt (GEEN vraag)\n"
        "- Eén warme, concrete observatie over de PRAKTIJK zelf — gebruik echte "
        "datapunten (reviews, rating, plaatsnaam, specialisme, opvallend detail)\n"
        "- Noem de WEBSITE of een probleem NIET; de mail bruggt daar zelf naartoe. "
        "De opener is puur de warme, persoonlijke haak.\n"
        ...
        "VOORBEELDEN GOED (telkens ÉÉN zin, geen website, geen vraag):\n\n"
        "\"Plastische Chirurgie Groningen staat op 49 reviews met een strakke 5.0, dat "
        "bouw je niet op met gemiddelde zorg.\"\n\n"
        "\"Annebeth Kroeskop draait al jaren een sterke praktijk in Amsterdam, te zien "
        "aan die 50 reviews met 4.9 sterren.\"\n\n"
        "\"Met een actieve Instagram-lijn zet Avichi zich in Amsterdam duidelijk op de "
        "kaart, dat zie je niet vaak bij klinieken.\"\n\n"
        ...
        "Antwoord nu met alleen die ene openingszin voor de meegegeven lead."
    )
